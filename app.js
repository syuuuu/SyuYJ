...
app.get('/add-blog', (req, res) => {
    const blog = new Blog({
        title: ' 畢業感想',
        content: '現在我也已經大四快畢業了也慢慢的知道自己的興趣在哪裡，目前最有興趣的是當前端工程師或是後端工程師。現在我的實力看起來真的沒特別厲害，但我會自己慢慢增加實力讓自己可以當獨當一面的工程師。很感謝我大學這四年來教導我的每位老師。'
    });
...
app.get('/blogs', (req, res) => {
    Blog.find().then(result => {
        res.render('index', { blogs: result, title: "所有新聞" });
    }).catch(err => {
        console.log(err);
    });
});
